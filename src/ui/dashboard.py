...
def render_dashboard(stats: Dict):
    ...
    with col3:
        retriever_primary = (
            max(stats["retriever_usage"].items(), key=lambda x: x[1], default=("N/A", 0))[0]
            if stats["retriever_usage"]
            else "N/A"
        )
        st.metric(label="Récupérateur Principal", value=retriever_primary.upper())

    with col4:
        system_uptime = _calculate_uptime(stats["system_info"].get("first_started"))
        st.metric(label="Disponibilité", value=system_uptime)
...
def _render_weekly_trend_chart(weekly_stats: List[Dict]):
    ...
    fig.update_layout(
        ...
        yaxis=dict(showgrid=True, gridcolor="rgba(0,0,0,0.05)", title="Nombre de Requêtes"),
        hovermode="x unified",
        showlegend=False,
    )
...
def _render_recent_searches(recent_searches: List[Dict]):
    ...
    for search in recent_searches[:10]:
        ...
        display_query = (
            search["query"][:60] + "..." if len(search["query"]) > 60 else search["query"]
        )
