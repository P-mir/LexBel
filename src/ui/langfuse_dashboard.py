...
def render_langfuse_dashboard(stats: dict):
    ...
    with col2:
        ...
        st.metric("Total Cost", f"${cost:.4f}", help="Cumulative API cost")

    with col3:
        tokens = stats.get("total_tokens", 0)
        st.metric("Total Tokens", f"{tokens:,}", help="Sum of input + output tokens")
    ...
    with col1:
        st.metric("Average", f"{stats.get('avg_latency_ms', 0):.0f}ms", help="Mean response time")
