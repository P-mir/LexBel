...
Reformule cette question en une question autonome et complète qui peut être comprise sans l'historique de la conversation. Garde le même contexte juridique et la même langue (français).
...
5. Sois précis et concis"""
    ...
    def query_stream(
    ...
    ) -> Generator[tuple[str, Optional[list], Optional[dict]], None, None]:
        ...
        if tracer.enabled:
            trace = tracer.create_trace(
                name="conversational_rag_query_stream",
                session_id=session_id,
                metadata={
                    "thread_id": thread_id,
                    "retriever_type": type(self.retriever).__name__,
                ...
                tags=["rag", "conversational", "legal", "stream"],
            )
        ...
        if not sources:
            yield (
                "Je n'ai pas trouvé d'articles pertinents pour répondre à cette question.",
                [],
                {"num_sources": 0},
            )
            if trace:
...
5. Sois précis et concis"""
        ...
        try:
            ...
            retrieval_details = {
                ...
                "total_tokens": cost_info.get("input_tokens", 0)
                + cost_info.get("output_tokens", 0),
                "retrieval_time_ms": retrieval_duration * 1000,
                "generation_time_ms": generation_duration * 1000,
                "total_time_ms": (time.time() - query_start_time) * 1000,
            }

            yield ("", sources, retrieval_details)
            ...
            yield (f"Erreur lors de la génération de la réponse: {e}", [], {"error": str(e)})
