...
class LexBelAnalytics:
    ...
    def get_dashboard_stats(self) -> Dict:
        ...
        if self.data["searches"]:
            avg_retrieval = sum(s["retrieval_time_ms"] for s in self.data["searches"]) / len(
                self.data["searches"]
            )
        else:
            avg_retrieval = 0
        ...
        popular = sorted(self.data["popular_queries"].items(), key=lambda x: x[1], reverse=True)[
            :10
        ]
    ...
    def _get_weekly_stats(self) -> List[Dict]:
        ...
        for i in range(7):
            ...
            day_stats = self.data["daily_stats"].get(date, {"queries": 0, "avg_retrieval_ms": 0})
            stats.append(
                {
                    "date": date,
    ...
    def clear_old_data(self, days: int = 30):
        ...
        # Fillter searches
        self.data["searches"] = [s for s in self.data["searches"] if s["timestamp"] > cutoff_iso]

        self.data["performance"] = [
            p for p in self.data["performance"] if p["timestamp"] > cutoff_iso
        ]
